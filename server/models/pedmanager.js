...
class PedManager {
    constructor() {
        ...
        this.ticker = new mp.Event("server:Tick", () => {
            this.tick();
        });
    }
    tick() {
        this._pedPool.forEach(syncPed => {
            ...
            mp.players.forEachInRange(position, this._syncRange, (player) => {
                if (position.dist(player.position) < nearest_pos) {
                    if (player.packetLoss < old_packagelost) {
                        old_packagelost = player.packetLoss;
                        best_syncer = player;
                        nearest_pos = position.dist(player.position);
                    }
                }
    ...
    addPed(SyncPed) {
        console.log("new sync ped", this.name);
